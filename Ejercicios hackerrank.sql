...
/*
Cuando en SQL pones dos tablas (o subconsultas) en el FROM separadas por una coma, estás haciendo lo que en matemáticas se llama un Producto Cartesiano (también conocido como CROSS JOIN). 
Básicamente, SQL agarra cada fila de la primera caja y la combina con todas las filas de la segunda caja.
*/
...
SELECT
Inicios.Start_Date,
MIN(Finales.End_Date)
FROM 
    ( -- Saca los Inicios Reales
      SELECT Start_Date 
      FROM Projects 
      WHERE Start_Date NOT IN (SELECT End_Date FROM Projects)
    ) AS Inicios,
    
    ( -- Saca los Finales Reales
      SELECT End_Date 
      FROM Projects 
      WHERE End_Date NOT IN (SELECT Start_Date FROM Projects)
    ) AS Finales

WHERE Inicios.Start_Date < Finales.End_Date
GROUP BY Inicios.Start_Date
ORDER BY (MIN(Finales.End_Date) - Inicios.Start_Date) ASC, Inicios.Start_Date ASC
